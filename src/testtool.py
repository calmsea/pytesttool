...
reports.setAttribute(u'start', time.strftime("%Y-%m-%dT%H:%M:%S"))

tests = Tests()
tests.parse(testroot.getElementsByTagName(u'tests')[0])
print tests.__dict__
report_list = tests.run()

for report in report_list:
    reports.appendChild(report.toXmlElements(reportdoc))

reports.setAttribute(u'end', time.strftime("%Y-%m-%dT%H:%M:%S"))
