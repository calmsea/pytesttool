from cmd import Cmd
import readline
from testtool import TestTool, Test, Tests
...
class TestToolCmd(Cmd):
    ...
    def add_test(self, str):
        print "add Test(%s)" % str
        items = str.split()
        param = dict(zip(items[0::2], items[1::2]))
        print param
        test = Test(**param)
        if param.has_key("clause"):
            clause = map(int, param.get("clause").split("-"))
        else:
            clause = []
        self.testtool.addTest(test, clause)
    def add_clause(self, str):
        print "add Clause(%s)" % str
        items = str.split()
        param = dict(zip(items[0::2], items[1::2]))
        print param
        tests = Tests(**param) 
        if param.has_key("caluse"):
            clause = map(int, param.get("clause").split("-"))
        else:
            clause = []
        self.testtool.addClause(tests, clause)
